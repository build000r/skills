...
**When to use:** the user wants a single image generated by an external image-creation tool (ChatGPT image creation via Oracle, or pasted into a similar surface). N = 1 image, but the depth-per-field discipline of the deep-research contract still applies.

**Shape:**

- Subject (concrete, visualizable, mid-action)
- Composition and framing (shot type, angle, focal length character, depth of field, subject placement, foreground/midground/background)
- Setting and environment
- Lighting (source, direction, quality, color temperature, shadow behavior, mood)
- Style and medium (named, committed — photographic film stock / illustration tradition / 3D render aesthetic / painting medium)
- Color palette (dominant, accent, harmony, saturation, contrast)
- Mood and atmosphere
- Detail and texture
- Aspect ratio and orientation
- Text in image (explicit "no text" or exact text in quotes)
- Hard constraints (drift items to avoid, style boundary, behavior for unspecified regions)
- What to return (image plus a verification caption)

**Anti-pattern:** an image prompt of the form "a beautiful photo of X in Y style." Image generators read those as license to fall back to stock-pose defaults. Force the layered specification — composition, lighting, palette, atmosphere — even when it feels redundant. The redundancy is what defeats the default mode.

**Red flag to call out:** the model silently substituting a generic "AI photography" look for the named style. Require the verification caption so substitutions surface instead of hiding in a polished render.

Full template at `assets/templates/image-creation.md`.
