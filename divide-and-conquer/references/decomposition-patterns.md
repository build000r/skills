...
- [ ] Bounded task-runner nodes such as mechanical scripting, fixtures/docs
      cleanup, generated-command cleanup, deterministic codemods, or scoped
      `$commit` batching use the selected `low` route when the Bead names
      exact writes, validation, stop rules, review owner, and final authority
- [ ] Ordinary implementation needing judgment uses the selected `med` route;
      a multi-file node alone does not require `high`
- [ ] Design-related nodes use the selected design route, with visual proof
      and `high` tier final acceptance
- [ ] Runtime orchestration uses the selected `low` route. No-ragrets bead
      composition, decomposition/synthesis, system design, impactful execution,
      integration review, commit acceptance, and final-say nodes use `high`
      through the retained SBP route-v2 decision
