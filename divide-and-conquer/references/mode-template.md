...
- **Default model strategy**: route design-related nodes to Claude Opus; route all other nodes to Codex `gpt-5.5`; default to `high`, reserve `medium` for clearly bounded read-only work, use `xhigh` for review or ambiguity, and round up when unsure
