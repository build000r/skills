...
## After the README: Competitive Landscape Research

The README is the sales pitch. Once it ships, the next job is honest positioning. Use `gh` to pull live signals from the repos your project sits near, then advise the user on where the project actually stands and where to shift.

Read [COMPETITIVE-LANDSCAPE.md](references/COMPETITIVE-LANDSCAPE.md) for the full workflow, `gh` commands, and templates.

### Quick version

```bash
# 1. Pull stars/issues/activity from comparable repos
gh api repos/OWNER/REPO --jq '{stars: .stargazers_count, open_issues: .open_issues_count, pushed: .pushed_at}'

# 2. Sample recent issues for pain signals
gh issue list --repo OWNER/REPO --state open --limit 20 --json title,labels,createdAt

# 3. Build a comparison table and market map from live data
# 4. Write a VISION.md or inline the positioning into the README
```

### What to do with the findings

After the README is written and the landscape is mapped, circle back to the user with an honest take:

1. **Where does this project actually sit?** — Not where we wish it sat. Map it against the real alternatives on axes that matter (platform heft vs agent-native focus, or whatever fits).
2. **What are users of adjacent tools actually struggling with?** — Pull real issue titles. These are the gaps this project either fills or doesn't.
3. **Where could this project build on the backs of giants?** — Identify existing tools, libraries, or infrastructure the project could compose with rather than rebuild. Be specific: "use X for Y instead of rolling your own."
4. **If we want to truly add value, consider shifting in these directions** — Name 2-3 concrete directional bets based on what the landscape research surfaced. Not vague aspirations — specific gaps that the competitive data shows are underserved.

This is not a demoralizing exercise. It is the difference between a README that makes claims and a project that earns them. The user should walk away knowing exactly what is real, what is aspirational, and what to build next.

---

## Strategic Depth: When the Project Deserves a VISION.md

Some projects have more strategic context than a README can carry. When the competitive research surfaces real positioning nuance — wedge arguments, non-goals, market maps, evidence from comparable repos — extract it into a companion `docs/VISION.md`.

Read [EXTENDED-TEMPLATES.md](references/EXTENDED-TEMPLATES.md) for the VISION.md template.

### What goes in VISION.md vs the README

| Content | Where |
|---------|-------|
| Mission, vision, values | VISION.md |
| Competitive fit table with detailed rationale | VISION.md |
| Market map (positioning chart) | VISION.md |
| Evidence from comparable repos (issue signals) | VISION.md |
| Strategic non-goals | VISION.md |
| "Who it is for / not for" with nuance | VISION.md |
| Build-vs-buy analysis and directional bets | VISION.md |
| One-paragraph competitive summary + comparison table | README |
| Link to VISION.md for deeper thesis | README |

### The README links to it, not the other way

The README says: *"For the deeper thesis, see [docs/VISION.md](docs/VISION.md): mission, vision, values, competitive fit, and why this project intentionally stops short of X, Y, and Z."*

The VISION.md does not need to sell. It needs to be honest. It is the document where you say "this project exists because of this specific gap, here is the evidence, and here are the things we will not become."
...
```
□ Hero section with illustration + badges + one-liner + install path
□ TL;DR with Problem/Solution/Feature table
□ Quick example (5-10 commands)
□ Installation methods documented (whatever applies to the project)
...
After writing, run the validation script to catch structural issues:

```bash
scripts/validate_readme.sh path/to/README.md
```

This checks: required sections present, badge syntax, code block fences closed, contributions text matches, and no broken internal links.
...
| **GH CLI competitive research workflow** | [COMPETITIVE-LANDSCAPE.md](references/COMPETITIVE-LANDSCAPE.md) |
