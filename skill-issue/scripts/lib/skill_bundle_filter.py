...
def _matches_patterns(rel_path: str, patterns) -> bool:
    ...
    for pattern in patterns:
        ...
        if not normalized_pattern:
            continue
        root_anchored = normalized_pattern.startswith("/")
        if normalized_pattern.startswith("/"):
            normalized_pattern = normalized_pattern.lstrip("/")
            if not normalized_pattern:
                continue
        if root_anchored:
            if normalized_rel_path == normalized_pattern or normalized_rel_path.startswith(f"{normalized_pattern}/"):
                return True
            if fnmatch(normalized_rel_path, normalized_pattern):
                return True
            continue
